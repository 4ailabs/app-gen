...
export function PersonNode({ data, selected }: { data: NodeData; selected: boolean }) {
  ...
  const getShapeClasses = (gender: string) => {
    switch (gender) {
      case 'male':
        return 'rounded-none aspect-square w-16 bg-[var(--surface)] border-[3px] border-[var(--text-2)]';
      case 'female':
        return 'rounded-full aspect-square w-16 bg-[var(--surface)] border-[3px] border-[var(--text-2)]';
      case 'unknown':
        return 'rotate-45 aspect-square rounded-sm w-12 bg-[var(--surface)] border-[3px] border-[var(--text-2)]';
      case 'pregnancy':
        return 'w-14 aspect-square'; // Wrapper only
      case 'miscarriage':
        return 'rounded-full aspect-square w-6 bg-[var(--text-2)]';
      case 'abortion':
        return 'rounded-full aspect-square w-6 bg-[var(--text-2)] relative';
      case 'stillbirth':
        return 'rounded-none aspect-square w-16 bg-[var(--surface)] border-[3px] border-[var(--text-2)]';
      default:
        return 'rounded-none aspect-square w-16 bg-[var(--surface)] border-[3px] border-[var(--text-2)]';
  ...
  return (
    <div className={cn(
      ...
      <div className="absolute -top-10 text-center w-max whitespace-nowrap z-20">
        <span className="text-xs font-bold uppercase tracking-tighter text-[var(--text)] bg-[var(--surface)]/90 px-2 py-0.5 rounded shadow-sm backdrop-blur-md border border-[var(--border)]">
          {data.name || 'Desconocido'} {age !== null && <span className="text-[var(--text-3)] font-normal ml-1">({age})</span>}
        </span>
      </div>
      ...
      <div className={cn(
        ...
        selected && !isSpecialShape ? 'shadow-2xl scale-110 ring-8 ring-[var(--bg)]' : 'hover:scale-105 shadow-md',
        selected && isSpecialShape ? 'scale-125 drop-shadow-xl' : '',
        !isSpecialShape && data.status === 'deceased' && 'bg-[var(--surface-2)]'
      )}>
        {/* Pregnancy Triangle SVG */}
        {data.gender === 'pregnancy' && (
          <svg viewBox="0 0 24 24" className="absolute inset-0 w-full h-full text-[var(--text-2)]">
            <polygon points="12,2 22,22 2,22" fill={selected ? "var(--bg)" : "var(--surface)"} stroke="currentColor" strokeWidth="2.5"/>
          </svg>
        )}

        {/* Abortion Diagonal SVG */}
        {data.gender === 'abortion' && (
          <svg viewBox="0 0 24 24" className="absolute inset-[-12px] w-[calc(100%+24px)] h-[calc(100%+24px)] text-[var(--terra)] pointer-events-none">
        ...
        {(isDeceased || data.gender === 'stillbirth') && data.gender !== 'abortion' && data.gender !== 'pregnancy' && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-hidden rounded-[inherit]">
            <div className="absolute w-[150%] h-[2.5px] bg-[var(--text-2)] rotate-45" />
            <div className="absolute w-[150%] h-[2.5px] bg-[var(--text-2)] -rotate-45" />
          </div>
        )}
        ...
        <div className={cn(
          ...
          {data.hasUnknownInfo ? (
            <span className="font-serif italic text-3xl font-bold text-[var(--text-3)]">
              ?
            </span>
          ) : !isSpecialShape && getInitials(data.name) ? (
            <span className="font-serif italic text-xl text-[var(--text-2)]/80 mt-1">
      ...
      {(data.birthDate || data.deathDate || data.birthDateUnknown || data.deathDateUnknown || data.status === 'deceased') && (
        <div className="absolute -bottom-8 text-center w-max whitespace-nowrap">
          <span className="text-[10px] text-[var(--text-3)] font-bold tracking-widest">
      ...
      {data.isGenosociogramMode && hasTraumaEvents && (
        <div className="absolute -right-3 -top-3 bg-[var(--terra)] rounded-full p-1 shadow-lg ring-4 ring-[var(--bg)]">
      ...
      <button 
        ...
        className={cn(
          "absolute -right-12 top-1/2 -translate-y-1/2 p-2 bg-[var(--surface)] border border-[var(--border)] rounded-full text-[var(--text-3)] hover:text-[var(--text-2)] hover:bg-[var(--bg)] hover:shadow-lg shadow-sm transition-all duration-200 nodrag cursor-pointer",
